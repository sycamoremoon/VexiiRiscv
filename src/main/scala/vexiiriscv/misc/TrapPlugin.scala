    ...
    val harts = for(hartId <- 0 until HART_COUNT) yield new Area{
      ...
      val interrupt = new Area {
        ...
        if (priv.p.withHypervisor) {
          ...
          privilegeAllowInterrupts += PrivilegeMode.VS -> ((csr.s.status.sie && !csr.withHostPrivilege) || !csr.withVirtualSupervisorPrivilege)
        }
      ...
      val trap = new Area {
        ...
        val fsm = new StateMachine {
          ...
          if(ats.mayNeedRedo) ATS_RSP.whenIsActive{
            when(atsPorts.refill.rsp.valid){
              ...
              when(atsPorts.refill.rsp.pageFault || atsPorts.refill.rsp.accessFault){
                ...
                switch(atsPorts.refill.rsp.accessFault ## atsPorts.refill.rsp.accessFault ## pending.state.arg(1 downto 0)){
          ...
          TRAP_TVAL.whenIsActive {
            ...
            if (priv.p.withSupervisor) {
              addressMapping += PrivilegeMode.S -> (
                if (priv.p.withHypervisor)
                  Mux(
                    csr.privilege === S(PrivilegeMode.VS) ||
                    csr.privilege === S(PrivilegeMode.VU),
                    csr.h.tval.getAddress(),
